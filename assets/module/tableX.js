...
layui.define(["layer", "table", "laytpl", "contextMenu"], function (f) {
    ...
    a.bindCtxMenu = function (t, r) {
        ...
        g(s).bind("contextmenu", function (w) {
            var x = g(this);
            g(s).removeClass("layui-table-click");
            x.addClass("layui-table-click");
            var v;
            if (typeof r === "function") {
                v = r(u[x.data("index")], w.currentTarget)
            } else {
                v = r
            }
            ...
            function y(B) {
                ...
                for (var A = 0; A < B.length; A++) {
                    z.push({
                        icon: B[A].icon, name: B[A].name, _click: B[A].click, click: function (E, D) {
                            var C = g(D.currentTarget);
                            this._click && this._click(u[C.data("index")], D.currentTarget);
                            C.removeClass("layui-table-click")
                        }, subs: y(B[A].subs)
                    })
                }
                return z
            ...
            e.show(y(v), w.clientX, w.clientY, w);
            return false
        })
    };
    ...
    a.render = function (t) {
        ...
        q.on("sort(" + r + ")", function (w) {
            ...
            var v = g.extend(t.where, {sort: u, order: x});
    ...
    var j = function (s) {
        var r = s.config.id, t = g("input[" + h + '="' + r + '"]');
        if (!(t && t.length > 0)) {
            return
        }
        ...
        t.off("keydown").on("keydown", function (x) {
            ...
            var v = i.msg("搜索中..", {icon: 16, shade: 0.01, time: 0});
            var w = a.getTbData(r);
    ...
    var l = function (s) {
        ...
        q.on("sort(" + r + ")", function (w) {
            ...
            var u = i.msg("加载中..", {icon: 16, shade: 0.01, time: 0});
        ...
        g("[" + b + '="' + r + '"]').off("click").on("click", function (t) {
            ...
            g(".tbx-dropdown-menu").off("click").on("click", ".tbx-dropdown-menu-item", function (x) {
                ...
                if (w === "check") {
                    ...
                    if (v.data.length === 0) {
                        i.msg("请选择要导出的数据", {icon: 2})
                    } else {
                        g(".tbx-dropdown-menu").remove();
                        a.exportData({fileName: s.config.title, cols: s.config.cols, data: v.data})
                    }
                } else {
                    if (w === "current") {
                        a.exportData({fileName: s.config.title, cols: s.config.cols, data: q.cache[r]})
                    } else {
                        if (w === "all") {
                            a.exportData({fileName: s.config.title, cols: s.config.cols, data: a.getTbData(r)})
                        }
                    }
                }
                if (x !== void 0) {
